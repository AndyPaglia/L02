...
import sys
sys.path.append("/content/drive/MyDrive/lp-music-caps")  # Sostituisci con il percorso corretto
...
def captioning(args, output_file):
    save_dir = "/content/drive/MyDrive/lp-music-caps/lpmc/music_captioning/exp/transfer/lp_music_caps/" # percorso corretto
    config = OmegaConf.load(os.path.join(save_dir, "hparams.yaml"))
    model = BartCaptionModel(max_length=config.max_length)
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    checkpoint = torch.load("/content/drive/MyDrive/lp-music-caps/exp/transfer/lp_music_caps/last.pth", map_location=device)
